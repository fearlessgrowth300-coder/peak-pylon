...
function LiveNowCommunityView({ members, onPick }: { members: Member[]; onPick: (member: Member) => void }) {
  return <div className="space-y-6 px-4 py-6"><header><h1 className="text-3xl font-black">🔴 LIVE NOW</h1><p className="mt-1 text-sm text-muted-foreground">Creators currently live from your community.</p></header><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{members.map((member) => { const login = member.handle.replace(/^@/, ""); return <article key={member.id} className="overflow-hidden rounded-2xl border border-border bg-popover hover:border-primary"><button onClick={() => onPick(member)} className="w-full text-left">{member.platform.toLowerCase() === "twitch" && login ? <iframe title={`${member.name} live stream`} src={`https://player.twitch.tv/?channel=${encodeURIComponent(login)}&parent=peak-pylon.vercel.app`} className="h-44 w-full border-0 bg-black" allowFullScreen /> : <div className="h-44 bg-accent bg-cover bg-center" style={member.banner ? { backgroundImage: `url(${member.banner})` } : undefined}/>}<span className="relative -top-40 m-3 inline-block rounded bg-destructive px-2 py-1 text-xs font-black text-white">LIVE</span><div className="p-4"><div className="flex items-center gap-3"><Avatar member={member} size={44}/><div><p className="font-bold">{member.name}</p><p className="text-xs text-muted-foreground">{member.handle}</p></div></div><p className="mt-3 line-clamp-2 text-sm text-muted-foreground">{member.bio || "Live community creator"}</p></div></button></article>; })}</div>{!members.length && <p className="text-sm text-muted-foreground">No community creators are live right now.</p>}</div>;
}

function TrendingCommunityView({ posts, members, onPick, isAdmin, onCreate }: { posts: Post[]; members: Map<string, Member>; onPick: (member: Member) => void; isAdmin: boolean; onCreate: (post: Pick<PostInput, "text" | "image">) => Promise<void> }) {
  const [draft, setDraft] = useState(""); const [title, setTitle] = useState(""); const [image, setImage] = useState<File | null>(null); const [busy, setBusy] = useState(false);
  return <div className="space-y-5 px-4 py-6"><header><h1 className="text-3xl font-black">🔥 TRENDING</h1><p className="mt-1 text-sm text-muted-foreground">Admin announcements and conversations across the community.</p></header>{isAdmin && <form onSubmit={async (event) => { event.preventDefault(); if (!title.trim() && !draft.trim()) return; setBusy(true); try { const uploaded = image ? await uploadCommunityMedia(image) : ""; await onCreate({ text: title.trim() ? `${title.trim()}\n\n${draft.trim()}` : draft.trim(), image: uploaded }); setTitle(""); setDraft(""); setImage(null); } finally { setBusy(false); } }} className="rounded-2xl border border-primary/40 bg-popover p-4"><p className="mb-2 text-xs font-bold uppercase tracking-wider text-primary">Create trending post · admin only</p><input value={title} onChange={(event) => setTitle(event.target.value)} className="mb-2 w-full rounded-lg bg-input px-3 py-2 text-sm outline-none" placeholder="Post title"/><textarea value={draft} onChange={(event) => setDraft(event.target.value)} rows={4} className="w-full rounded-lg bg-input px-3 py-2 text-sm outline-none" placeholder="Write the announcement or discussion…"/><div className="mt-2 flex items-center justify-between gap-2"><input type="file" accept="image/*" onChange={(event) => setImage(event.target.files?.[0] ?? null)} className="min-w-0 text-xs"/><button disabled={busy} className="rounded-lg bg-primary px-4 py-2 text-sm font-bold text-primary-foreground disabled:opacity-50">{busy ? "Publishing…" : "Publish"}</button></div></form>}<div className="space-y-4">{posts.map((post) => { const author = members.get(post.authorId); return <article key={post.id} className="rounded-2xl border border-border bg-popover p-5"><button onClick={() => author && onPick(author)} className="flex items-center gap-3 text-left"><Avatar member={author ?? { name: "Community", avatar: "", status: "offline" }} size={42}/><span><strong>{author?.name ?? "Community"}</strong><span className="ml-2 text-xs text-muted-foreground">{new Date(post.time).toLocaleString()}</span></span></button><p className="mt-4 whitespace-pre-wrap text-base leading-relaxed">{post.text}</p>{post.image && <img src={post.image} alt="Trending attachment" className="mt-4 max-h-96 w-full rounded-xl object-cover"/>}<div className="mt-4 flex gap-5 border-t border-border pt-3 text-sm text-muted-foreground"><span>♡ Like</span><span>◌ Comment</span><span>↗ Share</span></div></article>; })}</div>{!posts.length && <p className="text-sm text-muted-foreground">No admin trending posts yet.</p>}</div>;
}
