      ...
      <main className="flex min-w-0 flex-1 flex-col">
        ...
        <div className="flex min-h-0 flex-1">
          ...
          <div ref={scrollRef} className="min-w-0 flex-1 overflow-y-auto">
            ...
            {view === "trending" && <TrendingCommunityView posts={state.posts.filter((post) => post.channel === "trending")} members={memberById} isAdmin={isAdmin} onCreate={async (text) => { const authorId = myAccount?.id ?? adminMembers[0]?.id; if (authorId) await addPost({ authorId, text, channel: "trending" }); }} onPick={setProfile} />}
...
function TrendingCommunityView({ posts, members, onPick, isAdmin, onCreate }: { posts: Post[]; members: Map<string, Member>; onPick: (member: Member) => void; isAdmin: boolean; onCreate: (text: string) => Promise<void> }) {
  const [draft, setDraft] = useState("");
  return <div className="space-y-5 px-4 py-6"><header><h1 className="text-3xl font-black">🔥 TRENDING</h1><p className="mt-1 text-sm text-muted-foreground">Admin announcements and conversations across the community.</p></header>{isAdmin && <form onSubmit={(event) => { event.preventDefault(); if (draft.trim()) void onCreate(draft.trim()).then(() => setDraft("")); }} className="rounded-2xl border border-primary/40 bg-popover p-4"><p className="mb-2 text-xs font-bold uppercase tracking-wider text-primary">Admin post</p><div className="flex gap-2"><textarea value={draft} onChange={(event) => setDraft(event.target.value)} required rows={2} className="min-w-0 flex-1 rounded-lg bg-input px-3 py-2 text-sm outline-none" placeholder="Share an announcement with the community…"/><button className="rounded-lg bg-primary px-4 py-2 text-sm font-bold text-primary-foreground">Post</button></div></form>}<div className="space-y-4">{posts.map((post) => { const author = members.get(post.authorId); return <article key={post.id} className="rounded-2xl border border-border bg-popover p-5"><button onClick={() => author && onPick(author)} className="flex items-center gap-3 text-left"><Avatar member={author ?? { name: "Community", avatar: "", status: "offline" }} size={42}/><span><strong>{author?.name ?? "Community"}</strong><span className="ml-2 text-xs text-muted-foreground">{new Date(post.time).toLocaleString()}</span></span></button><p className="mt-4 whitespace-pre-wrap text-base leading-relaxed">{post.text}</p>{post.image && <img src={post.image} alt="Trending attachment" className="mt-4 max-h-96 w-full rounded-xl object-cover"/>}<div className="mt-4 flex gap-5 border-t border-border pt-3 text-sm text-muted-foreground"><span>♡ Like</span><span>◌ Comment</span><span>↗ Share</span></div></article>; })}</div>{!posts.length && <p className="text-sm text-muted-foreground">No admin trending posts yet.</p>}</div>;
}
